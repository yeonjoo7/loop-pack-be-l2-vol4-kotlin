...
- **정렬 조건 분기**: `likes_desc` 정렬은 likes 테이블과의 JOIN 또는 집계가 필요해 다른 정렬보다 쿼리 비용이 높다.
- **Soft Delete 필터**: 상품 조회 시 `deleted_at IS NULL` 조건이 항상 포함된다.
...
- **상품 존재 여부 검증**: 좋아요 등록 전 상품이 유효한지(삭제되지 않았는지) 확인한다.
...
- **스냅샷 저장**: OrderItem 생성 시 현재 상품의 이름과 가격을 복사해 저장.
- **결제 실패 보상**: 결제 실패 시 DB 트랜잭션은 이미 커밋되었으므로, 별도의 보상 트랜잭션으로 재고 복구.
